...
def parse_page_detail(detailUrlHtml):
    ...
    title = doc(
        'body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > div.company_info_wrap > div.info_lf > div.wrap_title > h1').text()  # 标题
    company = doc(
    ...
    region = doc(
        'body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > div.company_info_wrap > div.info_lf > ul > li:nth-child(3)').text().replace(
        "工作地点：", "")  # 地区
    # regionList = region1.split("-")
    strTime = doc(
        'body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > div.company_info_wrap > div.info_lf > div.wrap_title > div.wrap_title_rt > span').text().replace(
        "更新日期：", "") + " 00:00"  # 发布时间
    experience = doc(
        'body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > ul > li:nth-child(3)').text().replace("工作经验：",
                                                                                                            "")  # 工作经验
    people = doc('body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > ul > li:nth-child(8)').text().replace(
        "招聘人数：", "")  # 招收人数
    education = doc('body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > ul > li:nth-child(2)').text().replace(
        "学历要求：", "")  # 学历要求
    ...
    numberOfCompanies = doc(
        'body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > div.company_info_wrap > div.info_lf > ul > li:nth-child(2)').text().replace(
        "公司规模：", "")  # 公司人数
    companyNature = doc(
        'body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > div.company_info_wrap > div.info_lf > ul > li:nth-child(1)').text().replace(
        "公司性质：", "")  # 公司性质
    workplace1 = doc(
        'body > div.wrapper > div.wrap_lt.wrap_no > div.wrap_lt_job > div.company_info_wrap > div.info_lf > ul > li:nth-child(3)').text().replace(
        "工作地点：", "")  # 工作地址
    ...
    try:
        ...
        if time2 == today:
            # 执行sql
            countSql = 'select count(*) from jianzhuyingcaiwang where company = "%s"  and title = "%s"' % (
            company, title)
            ...
            if Count < 1:
                ...
                if isNull == None:
                    ...
                    updatePositionIdSql = "update tp_job_position p,jianzhuyingcaiwang r set r.positionid=p.positionid where r.positiontitle=p.title and company='%s' " % (
                        company)
...
if __name__ == '__main__':
    ...
    # 木工, 油漆工, 总工, 水电工程师, 造价员 砌筑工，建筑设计师，消防工程师  平面设计师 建筑设计师'
    #  kwlist = ['技术员', '质检员', '质量员', '测量员', '建造师', '预算员', '安全员', '资料员', '材料员', '施工员','建筑设计师']
    kwlist = ['技术员', '质检员', '质量员', '测量员', '建造师', '预算员', '安全员', '资料员', '材料员', '施工员', '总工', '造价员', '建筑设计师', '平面设计师',
              '消防工程师', '调度员']
    # kwlist = ['平面设计师','消防工程师']
