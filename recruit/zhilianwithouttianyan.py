...
def parse_page_detail(html, updateDates):
    ...
    doc.encode('latin-1').decode('unicode_escape')
    title = doc("#root > div.job-summary > div > div > h3").text()
    #title = doc("body > div.wrap > div.main > div.main1.cl.main1-top > div > ul > li.info-h3").text()
    # 标题
    ...
    company = doc(
        "#root > div:nth-child(4) > div.app-main__right > div.publisher > div > a").text().split(" ")[0] # 公司名称
    region = doc(
        '#root > div.job-summary > div > div > div.summary-plane__bottom > div.summary-plane__left > ul > li:nth-child(1) > a').text()  # 地区
    experience = doc(
        "#root > div.job-summary > div > div > div.summary-plane__bottom > div.summary-plane__left > ul > li:nth-child(2)").text()  # 工作经验
    education = doc(
        "#root > div.job-summary > div > div > div.summary-plane__bottom > div.summary-plane__left > ul > li:nth-child(3)").text()  # 学历
    people = doc(
        "#root > div.job-summary > div > div > div.summary-plane__bottom > div.summary-plane__left > ul > li:nth-child(4)").text()  # 招收人数

    # 精确到p标签
    # description1 = doc(
    #     'body > div.wrap > div.main > div.pos-info.cl > div.l.pos-info-in > div.responsibility.pos-common > div.pos-ul > p') # 任职要求

    jobhighlights = doc(".highlights__content").text()  # 福利待遇
    descriptions = doc(
        '.describtion__detail-content').text()  # 职位描述
    pattern = re.compile(' style=\"(.*?)\"')
    description = pattern.sub('', descriptions).replace('&#13;', '').replace('"', '')

    salary = doc("#root > div.job-summary > div > div > div.summary-plane__bottom > div.summary-plane__left > span").text()  # 薪资
    companyProfile = doc('.company__description').text()  # 公司简介
    # mainCompany = doc("").text()  # 公司主营
    mainCompany = ''
    companyNature = doc(
        '#root > div:nth-child(4) > div.app-main__right > div.publisher > div > div.company__detail > button.company__industry').text()  # 公司性质
    numberOfCompanies = doc('.company__size').text()  # 公司人数
    workplace = doc('.job-address__content-text').text()  # 工作地址
    webname = '智联'
    positionid = ""
    ...
    try:
        ...
        if strTime == today:
            ...
            if Count < 1:
                ...
                print(company)
                print(description)
                if description != '' or company.strip() or positiontitle.strip() or workplace.strip() or salary.strip() or workplace.strip():
                    insertSql = 'INSERT INTO zhilian (webname, title, company, positionid, positiontitle, experience, ' \
                                'salary, description, workplace, region, education, people, companyProfile, mainCompany, ' \
                                'companyNature, numberOfCompanies, jobHighlights, addtime, edittime, authstatus, authfailreason,' \
                                'authtime, hits, userid, regionid, latitude, longitude) VALUES ' \
                                '("%s", "%s", "%s", "%s", "%s", "%s", "%s", "\'%s\'", "%s", "%s", "%s", "%s", "%s", "%s", "%s", ' \
                                '"%s", \'%s\', "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s")' % (
                                    webname, title, company, positionid, positiontitle, experience, salary, description,
                                    workplace,
                                    region,
                                    education, people, companyProfile, mainCompany, companyNature, numberOfCompanies,
                                    jobhighlights,
                                    addtime,
                                    edittime, authstatus, authfailreason, authtime, hits, userid, regionid, latitude,
                                    longitude,
                                )
                ...
                if isNull == None:
                    ...
                    updatePositionIdSql = "update tp_job_position p,zhilian r set r.positionid=p.positionid " \
                                          "where r.positiontitle=p.title and company='%s' " % (
                                              company)
                ...
                delectsql = 'delete from zhilian where  description = ""or description is null or workplace is null or ' \
                            'workplace ="" or salary="" or length(description)<200'
                cursor.execute(delectsql)
                conn.commit()
...
if __name__ == '__main__':
    ...
    # 木工，  油漆工， 水暖工程师， 水电工程师， 造价员， 建筑设计师， 消防工程师  平面设计师 建筑设计师'
    # kwlist = ['技术员', '质检员', '质量员', '测量员', '建造师', '预算员', '安全员', '资料员', '材料员', '施工员', '总工', '造价员','建筑设计师', '消防工程师']    #  职业类型
    kwlist = ['质量员']
