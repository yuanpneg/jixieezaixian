...
def get_page_index(kw,page):
    ...
    indexUrl='https://search.51job.com/list/000000,000000,0000,26%252C51%252C09%252C50,1,99,'+kw+',2,'+page+'.html?lang=c&stype=1&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&lonlat=0%2C0&radius=-1&ord_field=0&confirmdate=9&fromType=5&dibiaoid=0&address=&line=&specialarea=00&from=&welfare='
    try:
...
def parse_page_detail(detailUrlHtml):
    ...
    title=doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tHeader.tHjob > div > div.cn > h1').text()# 标题
    company=doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tHeader.tHjob > div > div.cn > p.cname > a.catn').text()# 公司名称
    salary1 = doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tHeader.tHjob > div > div.cn > strong').text()
    salaryList = salary1.split("/")
    salary = salaryList[0]  # 薪资
    region1=doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tHeader.tHjob > div > div.cn > p.msg.ltype').text()
    ...
    description1 = doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tCompany_main > div:nth-child(1) > div > p')     # 任职要求
    descriptions = str(description1)
    pattern = re.compile(' style=\"(.*?)\"')
    description = pattern.sub('', descriptions).replace(' ', '').replace('	', '').replace('&#13;', '').replace('\'', '')
    companyProfile = doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tCompany_main > div:nth-child(3) > div').text()  # 公司简介
    mainCompany = doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tCompany_sidebar > div:nth-child(1) > div.com_tag > p:nth-child(3)').attr('title')  # 公司主营
    numberOfCompanies = doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tCompany_sidebar > div:nth-child(1) > div.com_tag > p:nth-child(2)').attr('title')  # 公司人数
    companyNature = doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tCompany_sidebar > div:nth-child(1) > div.com_tag > p:nth-child(1)').attr('title')  # 公司性质
    workplace1 = doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tCompany_main > div:nth-child(2) > div > p').text()  # 工作地址
    workplace=''
    if workplace1!='':
        workplaceList = workplace1.split("上班地址：")
        workplace = region+workplaceList[1]  # 地址
        workplace=workplace.replace("-","")
    jobhighlights=doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tHeader.tHjob > div > div.cn > div').text()
    company=doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tCompany_sidebar > div:nth-child(1) > div.com_msg > a > p').text()#公司
    positiontitle=doc('body > div.tCompanyPage > div.tCompany_center.clearfix > div.tHeader.tHjob > div > div.cn > h1').attr('title')#标题
        ...
        if time2 == today:
            ...
            if Count < 1 :
                ...
                if isNull == None:
                    ...
                    updatePositionIdSql = "update tp_job_position p,qiancheng r set r.positionid=p.positionid where r.positiontitle=p.title and company='%s' " % (company)
                ...
                delectsql = 'delete from qiancheng where telephone is null or telephone=""or description=""or description is null or workplace is null or workplace ="" or salary="" or positionid="" or length(description)<200'
                cursor.execute(delectsql)
                conn.commit()
